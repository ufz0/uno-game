...
export class Game {
  ...
  addPlayer(name, isBot = false, id = null) {
    const p = { id: id ?? `p${playerSeq++}`, name, isBot, hand: [], unoCalled: false, disconnected: false };
    this.players.push(p);
  ...
  start() {
    if (!this.canStart()) throw new Error('Need 2-10 players to start');
    this.status = 'playing';
    this.deck = buildDeck();
    this.discard = [];
  ...
  playCard(playerIndex, cardId, chosenColor = null) {
    ...
    if (isWild(card)) {
      if (!COLORS.includes(chosenColor)) return { ok: false, error: 'Pick a color for the wild card' };
  ...
  callUno(playerIndex) {
    ...
    if (p.hand.length !== 1) return { ok: false, error: 'You need exactly one card left to shout UNO' };
    p.unoCalled = true;
    return { ok: true };
  }
}
